...
use crate::{cube_n::space::{Face, Direction}, generic::{self, Movable, Alg}};

use super::{AxisMove, Amount};
...
impl From<Alg<AxisMove>> for Alg<QuarterAxisMove> {
    fn from(value: Alg<AxisMove>) -> Self {
		...
		for mov in value.moves {
			match mov.amount {
				Amount::Single => output_moves.push(QuarterAxisMove::new(mov.face, Direction::Positive)),
				Amount::Inverse => output_moves.push(QuarterAxisMove::new(mov.face, Direction::Negative)),
				Amount::Double => {
